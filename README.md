...
| `controller_bolag_logo_width.jsx` | Measures the source width of the logo layer matching the option selected in the "Bolag" dropdown on the null layer `controller`, multiplied by both the logo layer's own scale and the host layer's scale – i.e. the logo's actual on-screen width. Falls back to a layer named `Logo` if the selected layer doesn't exist. Apply to a 1D property (e.g. a Slider Control) on the layer the logo is parented to. |
...
| `controller_bolag_opacity.jsx` | Shows the single layer whose name matches the option selected in a Dropdown Menu Control named "Bolag" on the null layer `controller`, and hides all other company layers. Apply the same expression to the Opacity property of every company layer. The list of company names in the expression must match the dropdown's option order – AE dropdowns return an index, not a label. Layers not present in the list are left untouched. |
