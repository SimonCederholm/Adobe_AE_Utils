...
| `animated_chart_json.jsx` | ScriptUI panel that builds a single animated line chart from `data.json` (auto-selected if present). Creates year-named null layers with position expressions driven by a Controller null with Dropdown Menu Control, so switching series in the dropdown repositions the entire chart without keyframes. Adds a trim-path-animated shape line, a Tracker null following the line end, and a running value text label. Install under `Scripts/ScriptUI Panels/`. |
