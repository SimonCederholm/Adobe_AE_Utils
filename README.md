...
| File | Description |
|------|-------------|
| `controller_farg_rits_opacity.jsx` | Toggles visibility between two layers (`färg` and `rits`) from a Dropdown Menu Control named "Färg/Rits" on the null layer `controller` in the Main comp. Selecting one hides the other. Apply the same expression to the Opacity property of both layers – the active layer keeps its own opacity value, the other is forced to 0. |
| `controller_bolag_logo_width.jsx` | Measures the source width of the logo layer inside the `Bolag` precomp matching the option selected in the "Bolag" dropdown on the null layer `controller`, converted to Main-comp scale through the logo layer's own scale, the `Bolag` precomp layer's scale and the host layer's scale. Apply to a 1D property (e.g. a Slider Control) on a layer in the Main comp. |
| `controller_devis_opacity.jsx` | Three-state visibility toggle driven by a Dropdown Menu Control named "Devis" on the null layer `controller` in the Main comp: Ingen (nothing visible), A (only `Devis A`) or B (only `Devis B`). Apply the same expression to the Opacity property of both layers inside the `Devis` precomp. |
| `controller_bolag_opacity.jsx` | Shows the single layer whose name matches the option selected in a Dropdown Menu Control named "Bolag" on the null layer `controller` in the Main comp, and hides all other company layers. Apply the same expression to the Opacity property of every company layer inside the `Bolag` precomp. The list of company names in the expression must match the dropdown's option order – AE dropdowns return an index, not a label. Layers not present in the list are left untouched. |
| `controller_format_position.jsx` | Sets vertical position from the "Format" (4x5 / 9x16 / 16x9) and "Devis" dropdowns on the null layer `controller` in the Main comp. Horizontal position is left untouched (keeps the layer's own X by default, optionally locked to 1280); 9x16 without devis falls back to the layer's own values. Apply to the Position property. |
